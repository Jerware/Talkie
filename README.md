...
Speech library for Arduino/Teensy/Photon
...
Connect headphones or an audio amplifier to digital pin 3 on Arduino, or the DAC pin on Teensy & Photon.
...
The Audio output
----------------
Talkie sets up a special very high speed PWM, so audio can be taken directly from pin 3 with no other filtering.  On Teensy & Photon, a true analog output DAC is used.

Note that Talkie uses Timers 1 and 2 for PWM & timing, which may conflict with PWM outputs or other libraries.  On Teensy & Photon, one of the available IntervalTimers is used.
